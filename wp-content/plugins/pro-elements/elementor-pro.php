 ...
 * Description: This plugin enables some (GPL) features of Elementor Pro: Pro Widgets, Theme Builder templates, etc. Note that PRO Elements is not a substitute for Elementor Pro. Therefore we encourage you to <a href="https://elementor.com/pro/" target="_blank">purchase Elementor Pro</a> if possible.
...
define( 'ELEMENTOR_DISABLE_PRO', 'true' );
 ...
 * @return void
 */
function pro_elements_load_plugin() {
	load_plugin_textdomain( 'elementor-pro' );

	if ( ! did_action( 'elementor/loaded' ) ) {
		add_action( 'admin_notices', 'pro_elements_fail_load' );
	...
	if ( ! version_compare( ELEMENTOR_VERSION, $elementor_version_required, '>=' ) ) {
		add_action( 'admin_notices', 'pro_elements_fail_load_out_of_date' );
...
add_action( 'plugins_loaded', 'pro_elements_load_plugin' );

function pro_elements_clear_output($plugin) {
	if ($plugin == "elementor-pro/elementor-pro.php"){
		ob_clean();
	}
}
add_action( 'activate_plugin', 'pro_elements_clear_output');
 ...
 * @return void
 */
function pro_elements_fail_load() {
...
function pro_elements_fail_load_out_of_date() {
